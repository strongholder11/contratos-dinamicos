...
with col_form:
    ...
    with tab_pj:
        st.subheader("Dados da Pessoa Juridica")
        ...
        # Se SMART foi escolhido, mostrar opção de tipo de implantação
        tipo_implantacao_pj = None
        if 'SMART' in tipo_licenca_pj:
            tipo_implantacao_pj = st.selectbox(
                "Tipo de Implantação",
                options=['Smart', 'Clinic'],
                key="pj_implantacao"
            )
        
        # Encontrar o range de equipos para a licença escolhida
    ...
    with tab_pf:
        ...
        if 'SMART' in tipo_licenca_pf:
            tipo_implantacao_pf = st.selectbox(
                "Tipo de Implantação",
                options=['Smart', 'Clinic'],
                key="pf_implantacao"
            )
